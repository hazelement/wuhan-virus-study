...
class DataModel(object):
    ...
    def get_plot_data(self, y_threshold=None):
        ...
        for (x_, y_, new_y_, date_point_) in zip(
            self.target_data.x,
            self.target_data.total_cases,
            self.target_data.new_cases,
            self.target_data.date_point,
        ):
            if y_threshold:
                if y_ < y_threshold:
                    continue
        ...
        for index, row in event_data.iterrows():
            plot_event_data.append(
                [row["offset"].days, f"{row['location']}: {row['event']}"]
            )
    ...
    def plot_data(self, y_threshold=None, label_vertical=True, plot_accumulative=True):
        ...
        if plot_accumulative:
            plt.plot(
                plot_data.existing_x,
                plot_data.existing_y,
            ...
            for event in plot_data.events:
                ...
                plt.text(
                    event[0] + 0.1,
                    plot_data.existing_y[min(
                        len(plot_data.existing_y) - 1, event[0])],
                    event[1],
                    rotation=rotation,
                    color=color,
                )
    ...
    def plot_statistical_model(self):
        ...
        plt.plot(plot_data.curve_x, plot_data.curve_y,
                 label=self.target_data.label)
    ...
    def plot(self):
        ...
        if x_1 is not None and y_1 is not None:
            ...
            plt.text(1, y[-1] / 9,
                     self.statistic_model.fitted_formula, fontsize=12)
